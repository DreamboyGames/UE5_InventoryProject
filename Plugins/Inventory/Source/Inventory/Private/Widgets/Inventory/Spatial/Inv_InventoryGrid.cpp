...
void UInv_InventoryGrid::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
	...
	if (CursorExitedCanvas(CanvasPos, UInv_WidgetUtils::GetWidgetPosition(CanvasPanel), MousePos))
	{
		// Unhighlight slot
		return;
	}
...
void UInv_InventoryGrid::OnTileParametersUpdated(const FInv_TileParameters& Parameters)
{
	...
	if (CurrentQueryResult.ValidItem.IsValid() && GridSlots.IsValidIndex(CurrentQueryResult.ValidItemUpperLeftIndex))
	{
		...
		ChangeHoverType(CurrentQueryResult.ValidItemUpperLeftIndex, GridFragment->GetGridSize(), EInv_GridSlotState::GrayedOut);
	}
}
...
FInv_SpaceQueryResult UInv_InventoryGrid::CheckHoverPosition(const FIntPoint& Position, const FIntPoint& Dimensions)
{
	...
	if (OccupiedUpperLeftIndices.Num() == 1) // Single item at position - it's valid for swapping/combining
	{
		...
		Result.ValidItemUpperLeftIndex = GridSlots[Index]->GetUpperLeftIndex();
	}
...
FInv_SlotAvailabilityResult UInv_InventoryGrid::HasRoomForItem(const FInv_ItemManifest& Manifest)
{
	...
	for (const auto& GridSlot : GridSlots)
	{
		...
		if (!HasRoomAtIndex(GridSlot, GetItemDimensions(Manifest), Manifest.GetItemType(), MaxStackSize, CheckedIndices, TentativelyClaimedIndices))
		{
			continue;
		}
		...
		CheckedIndices.Append(MoveTemp(TentativelyClaimedIndices));
...
int32 UInv_InventoryGrid::GetStackAmount(const UInv_GridSlot* GridSlot) const
{
	...
	// If we are at a slot that doesn't hodl the stack count. We must get the actual stack count.
...
bool UInv_InventoryGrid::HasRoomAtIndex(const UInv_GridSlot* GridSlot, const FIntPoint& Dimensions, const FGameplayTag& ItemType, const int32 MaxStackSize, const TSet<int32>& CheckedIndices, TSet<int32> OutTentativelyClaimed)
{
	bool bHasRoomAtIndex = true;
	UInv_InventoryStatics::ForEach2D(GridSlots, GridSlot->GetIndex(), Dimensions, Columns, [&](const UInv_GridSlot* SubGridSlot)
	{
		if (CheckSlotConstraints(GridSlot, SubGridSlot, CheckedIndices, ItemType, MaxStackSize, OutTentativelyClaimed))
		{
...
bool UInv_InventoryGrid::CheckSlotConstraints(const UInv_GridSlot* GridSlot, const UInv_GridSlot* SubGridSlot, const TSet<int32>& CheckedIndices, const FGameplayTag& ItemType, const int32 MaxStackSize, TSet<int32>& OutTentativelyClaimed) const
{
	// Index claimed? 
...
UInv_SlottedItem* UInv_InventoryGrid::CreateSlottedItem(UInv_InventoryItem* Item, const int32 Index, const bool bStackable, const int32 StackAmount, const FInv_GridFragment* GridFragment, const FInv_ImageFragment* ImageFragment) const
{
